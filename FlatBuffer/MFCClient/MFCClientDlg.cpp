...
void CMFCClientDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT1, m_ctrlNick);
	DDX_Control(pDX, IDC_EDIT2, m_ctrlPort);
}
...
BOOL CMFCClientDlg::OnInitDialog()
{
	...
	m_ctrlPort.SetWindowTextW(_T("31400"));
...
void CMFCClientDlg::getNick()
{
	m_ctrlNick.GetWindowTextW(Nick);
}

void CMFCClientDlg::getPort()
{
	TCHAR buf[10] = { 0, };
	m_ctrlPort.GetWindowTextW(buf, 10);
	port = _ttoi(buf);
}
