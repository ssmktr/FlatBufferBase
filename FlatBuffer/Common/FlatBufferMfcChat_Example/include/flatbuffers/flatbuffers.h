...
template<typename T, typename IT> struct VectorIterator {
  ...
  VectorIterator(const uint8_t *data, uoffset_t i)
      : m_pData(data + IndirectHelper<T>::element_stride * i) {}
  VectorIterator(const VectorIterator &other) : m_pData(other.m_pData) {}
  VectorIterator() : m_pData(nullptr) {}
  ...
  difference_type operator-(const VectorIterator &other) const {
    return (m_pData - other.m_pData) / IndirectHelper<T>::element_stride;
  }

  // Note: return type is incompatible with the standard
  // `reference operator*()`.
  IT operator*() const { return IndirectHelper<T>::Read(m_pData, 0); }
  ...
  VectorIterator &operator++() {
    m_pData += IndirectHelper<T>::element_stride;
    return *this;
  }

  VectorIterator operator++(int) {
    VectorIterator temp(m_pData, 0);
    m_pData += IndirectHelper<T>::element_stride;
    return temp;
  }

  VectorIterator operator+(const uoffset_t &offset) const {
    return VectorIterator(m_pData + offset * IndirectHelper<T>::element_stride,
                          0);
  }

  VectorIterator &operator+=(const uoffset_t &offset) {
    m_pData += offset * IndirectHelper<T>::element_stride;
    return *this;
  }
...
template<typename T, uint16_t length> class Array {
 ...
 protected:
  ...
  uint8_t m_pData[length * sizeof(T)];
...
class Struct FLATBUFFERS_FINAL_CLASS {
 public:
  template<typename T> T GetField(uoffset_t o) const {
    return ReadScalar<T>(&m_pData[o]);
  }

  template<typename T> T GetStruct(uoffset_t o) const {
    return reinterpret_cast<T>(&m_pData[o]);
  }

  const uint8_t *GetAddressOf(uoffset_t o) const { return &m_pData[o]; }
  uint8_t *GetAddressOf(uoffset_t o) { return &m_pData[o]; }
...
class Table {
 public:
  const uint8_t *GetVTable() const {
    return m_pData - ReadScalar<soffset_t>(m_pData);
  }
  ...
  template<typename T> T GetField(voffset_t field, T defaultval) const {
    ...
    return field_offset ? ReadScalar<T>(m_pData + field_offset) : defaultval;
  }

  template<typename P> P GetPointer(voffset_t field) {
    auto field_offset = GetOptionalFieldOffset(field);
    auto p = m_pData + field_offset;
  ...
  template<typename P> P GetStruct(voffset_t field) const {
    auto field_offset = GetOptionalFieldOffset(field);
    auto p = const_cast<uint8_t *>(m_pData + field_offset);
  ...
  template<typename T> bool SetField(voffset_t field, T val, T def) {
    ...
    WriteScalar(m_pData + field_offset, val);
    return true;
  }
  ...
  bool SetPointer(voffset_t field, const uint8_t *val) {
    auto field_offset = GetOptionalFieldOffset(field);
    if (!field_offset) return false;
    WriteScalar(m_pData + field_offset,
                static_cast<uoffset_t>(val - (m_pData + field_offset)));
    return true;
  }

  uint8_t *GetAddressOf(voffset_t field) {
    auto field_offset = GetOptionalFieldOffset(field);
    return field_offset ? m_pData + field_offset : nullptr;
  }
  ...
  bool VerifyTableStart(Verifier &verifier) const {
    return verifier.VerifyTableStart(m_pData);
  }
  ...
  bool VerifyField(const Verifier &verifier, voffset_t field) const {
    ...
    return !field_offset || verifier.Verify<T>(m_pData, field_offset);
  }
  ...
  bool VerifyFieldRequired(const Verifier &verifier, voffset_t field) const {
    ...
    return verifier.Check(field_offset != 0) &&
           verifier.Verify<T>(m_pData, field_offset);
  }

  // Versions for offsets.
  bool VerifyOffset(const Verifier &verifier, voffset_t field) const {
    auto field_offset = GetOptionalFieldOffset(field);
    return !field_offset || verifier.VerifyOffset(m_pData, field_offset);
  }

  bool VerifyOffsetRequired(const Verifier &verifier, voffset_t field) const {
    auto field_offset = GetOptionalFieldOffset(field);
    return verifier.Check(field_offset != 0) &&
           verifier.VerifyOffset(m_pData, field_offset);
  }
